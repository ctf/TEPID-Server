...
class Migration(object):
	def __init__(self, applicable_types: Optional[List[str]], applicable_schema_versions: Optional[List[str]],
				 migration_function: Callable[[str], None], schema_version: str, db, ddoc):
		...
		while (len(self.ddoc.get_view(view_name)(skip=skip, limit=1)['rows']) > 0):
			...
			for row in to_migrate:
				...
				try:
					...
					skip += 1
					print("migration of " + doc[
						'_id'] + " was aborted due to type not matching the specification for this migration: " + str(
						e))
				except Exception as e:
					skip += 1
					print("migration of " + doc.get('_id') + " was aborted due to an error" + str(e))
